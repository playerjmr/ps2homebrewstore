---
hide:
  - navigation
  - toc
---

# Thanks!

<div class="grid cards" markdown>

-   __PS2 Wiki__

    ---

    ![PS2-Wiki_Pic](assets/ps2wiki.png){ height="150"}

    The inspiration for this fork, and the source of team who created such SAS/UMCS structure to make updating and supporting apps easy.

    [PS2WIKI](ps2wiki.github.io)

-   __PS2 Modchip Tutorials__

    ---

    ![PS2-Modchip-Tutorials_Pic](https://ps2modchiptutorials.com/assets/website_logo1.png){ height="150"}

    Another site that tries to follow such structure, but directed more to the lucky modchip master race.

    [PS2 Modchip Tutorials](ps2modchiptutorials.com)

-   __PS2 Space Discord Server__

    ---

    ![PS2-Space_Pic](assets/ps2space.png){ height="150"}

    Discord server full of great people willing to help!

    [Invite](https://discord.com/invite/JNEeD77R?utm_source=Discord%20Widget&utm_medium=Connect)

-   __Console Mods__

    ---

    ![Console-Mods_Pic](assets/consolemods.png){ height="150"}

    Wiki full of console information

    [Console Mods](consolemods.org)

-   __PS2 Dev Wiki__

    ---

    ![PS2-Dev-Wiki_Pic](assets/ps2devwiki.png){ height="150"}

    Wiki full of PS2 Specific information

    [PS2DevWiki](www.ps2devwiki.com)

-   __PS2 Homebrew Github__

    ---

    ![PS2-Homebrew-Github_Pic](assets/ps2homebrew-github.png){ height="150"}

    ps2homebrew is a collection of free tools for the Sony PlayStation 2® videogame system and similar hardware.

    [PS2Homebrew Github](https://github.com/ps2homebrew)



</div>


## No Thanks to:

!!! danger "No Thanks to:"

    TnA Plastic of r/PS2Homebrew, psx-place.com and PS2 Scene Discord.
